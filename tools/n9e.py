...
def register_n9e_tools(mcp: FastMCP):
    ...
    @mcp.tool(
        description="List all alerts generated by Nightingale in the past [hour/day/etc.]."
    )
    async def list_nightingale_alerts(duration: int) -> Dict[str, Any]:
        """
        Fetch alerts generated by Nightingale within the past `duration` seconds.

        Args:
            duration (int): Duration in seconds to look back from the current time.
        ...
        async with httpx.AsyncClient() as client:
            response = await client.get(
                f"{n9e_url}/api/n9e/alert-his", headers=headers, params=params
            )
